...
### 5. VH-026. 관리자 재처리 API 구현

운영자가 실패/timeout 인증을 재처리할 수 있는 내부 API를 구현한다.

검토할 정책:

- 재처리 가능한 상태
- terminal 상태 재처리 제한
- 기존 verification 재사용 여부
- provider 재선택 여부
- audit history 기록 방식

### 6. VH-027. 라우팅 정책 Admin API 구현
...
### 7. VH-028. Internal Mock Provider API 완성
...
### 8. VH-029. Metrics 및 Actuator 고도화
...
6. 운영 API와 문서는 API 흐름이 안정된 뒤 `VH-026`, `VH-027`, `VH-034` 순서로 정리한다.
